...
use bollard::models::{
    ContainerCreateBody, ContainerSummaryStateEnum, HostConfig, ImageSummary, PortBinding,
};
...
impl Manager {
    ...
    pub async fn exec_command<S: AsRef<str>>(
        &self,
        name: S,
        command: Vec<String>,
            ...
            StartExecResults::Attached { mut output, .. } => {
                ...
                while let Some(log) = output.try_next().await.map_err(|e| e.to_string())? {
                    if let LogOutput::StdOut { message } = log {
                        out += &format!("Output from exec: {}", String::from_utf8_lossy(&message));
                    }
                }
                return Ok(Some(out));
            }
    ...
    pub async fn container_status<S: AsRef<str>>(&self, name: S) -> Option<ContainerSummary> {
        let containers = self.list_containers().await.unwrap();
    ...
    pub async fn remove_container<S: AsRef<str>>(&self, name: S) -> Result<(), String> {
        self.docker
            .remove_container(
                name.as_ref(),
                Some(
