...
use crate::processing::station::Command::Stop;
...
impl SyncWorker {
    ...
    fn new(
        task: Box<dyn FnOnce(WorkerMeta) + Send + 'static>,
        args: WorkerArgs,
...
impl HybridThreadPool {
    ...
    pub(crate) fn control_sender(&self) -> Arc<Tx<Command>> {
        self.state.control_sender()
    }

    pub(crate) fn control_receiver(&self) -> Arc<Rx<Command>> {
        self.state.control_receiver()
    }

    pub(crate) fn send_control(&self, id: &usize, command: Command) {
        self.state.send_control(id, command);
    }

    pub(crate) fn stop(&self, id: &usize) {
        self.state.stop(id)
    }

    pub(crate) fn join(&self, id: &usize) {
        self.state.join(id)
    }
    ...
    pub fn execute_sync<F, S: AsRef<str>>(&self, name: S, f: F, depends_on: Vec<usize>) -> usize
    where
        F: FnOnce(WorkerMeta) + Send + 'static,
    {
...
impl PoolState {
    ...
    fn send_control(&self, num: &usize, command: Command) {
...
#[cfg(test)]
mod tests {
    use crate::util::threading::pool::HybridThreadPool;
    use std::sync::Arc;
    ...
    fn test_sync() {
        ...
        pool.execute_sync("main", |_args| println!("Hello world!"), vec![]);
    ...
    fn test_async() {
        ...
        pool.execute_async(
            "main",
            |_arg| Box::pin(async { println!("Hello world!") }),
            vec![],
        );
