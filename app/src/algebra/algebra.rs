...
impl Algebraic {
    ...
    pub(crate) fn calc_cost(&self, root: &AlgebraRoot) -> Cost {
        match self {
            ...
            Algebraic::Set(s) => root.get_children(s.id()).iter().fold(Cost::default(), |a, b| {
                let b = b.calc_cost(root);
                if a < b {
                    a
                } else {
                    b
    ...
    fn wrap(self) -> AlgebraRoot {
        AlgebraRoot::new(self)
    }
...
impl AlgInputDerivable for Algebraic {
    fn derive_input_layout(&self, root: &AlgebraRoot) -> Option<Layout> {
        match self {
            ...
            Algebraic::Set(s) => s.derive_input_layout(root),
...
impl AlgOutputDerivable for Algebraic {
    fn derive_output_layout(
        ...
        match self {
            ...
            Algebraic::Set(s) => s.initial.derive_output_layout(inputs, root),
...
impl Algebra for Algebraic {
    ...
    fn replace_id(self, id: usize) -> Self {
        match self {
            ...
            Algebraic::Set(s) => Algebraic::Set(s.replace_id(id)),
    ...
    fn derive_iterator(&self, root: &AlgebraRoot) -> Result<Self::Iterator, String> {
        Ok(match self {
            ...
            Algebraic::Set(s) => s.initial.derive_iterator(root)?,
