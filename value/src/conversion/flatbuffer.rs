use crate::{Array, Bool, Int, Text, Value, flatbuf as fb};
...
impl Value {
    ...
    pub(crate) fn to_fb_offset<'a>(
        &self,
        fbb: &mut FlatBufferBuilder<'a>,
    ) -> WIPOffset<fb::Value<'a>> {
        let (union_type, union_offset) = match self {
            ...
            Value::Dict(d) => {
                ...
                for (k, v) in d.iter() {
                    ...
                    entries.push(fb::DictEntry::create(
                        fbb,
                        &fb::DictEntryArgs {
                            key: Some(key_off),
                            value: Some(val_off),
            ...
            Value::Node(n) => {
                ...
                for l in &n.labels {
                    labels_off.push(fbb.create_string(&l.0));
                }
                ...
                for (k, v) in n.properties.iter() {
                    let key_off = fbb.create_string(k);
                    let val_off = v.to_fb_offset(fbb);
                    props_off.push(fb::DictEntry::create(
                        fbb,
                        &fb::DictEntryArgs {
                            key: Some(key_off),
                            value: Some(val_off),
            ...
            Value::Float(f) => {
                let data = fb::Float::create(fbb, &fb::FloatArgs { value: f.0.0 });
                (fb::ValueData::Float, data.as_union_value())
            }
            Value::Time(t) => {
                let data = fb::Time::create(fbb, &fb::TimeArgs { ms: t.ms, ns: t.ns });
            ...
            Value::Edge(e) => {
                let label = e
                    .label
                    .clone()
                    .map(|label| fbb.create_string(label.0.as_ref()));
                ...
                (fb::ValueData::Edge, data.as_union_value())
            }
        };
        ...
        fb::Value::create(
            ...
            &fb::ValueArgs {
                ...
                data: if union_type == fb::ValueData::NONE {
                    None
                } else {
                    Some(union_offset)
                },
    ...
    fn try_from(fb_val: fb::Value<'a>) -> anyhow::Result<Self> {
        match fb_val.data_type() {
            ...
            fb::ValueData::Node => {
                ...
                Ok(Value::node(Int(table.id()), labels, props.into()))
            }
            ...
            fb::ValueData::Edge => {
                ...
                let label = table.label().map(|label| Text(SmolStr::new(label)));
...
mod tests {
    ...
    fn test_message_round_trip() {
        ...
        let original_node = Value::node(
            Int(42),
            vec![Text(SmolStr::new("User")), Text(SmolStr::new("Admin"))],
            properties.into(),
        );
